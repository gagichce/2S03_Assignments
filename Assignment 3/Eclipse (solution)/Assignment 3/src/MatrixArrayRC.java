...
//November 2013
//2S03
//Matrix Array represented as arrays of rows of columns
...
public class MatrixArrayRC {
	...
	public MatrixArrayRC(Long[] inArray) {
		...
		for (int i = 0; i < inArray.length; i++) {
			if (i % dimension == 0)
				c++;
			Matrix[c][i % dimension] = inArray[i];
		}
	}
		...
		for (int i = 0; i < dimension; i++) {
			...
			if (Matrix[i][0] != 0) {
				...
				returns += Matrix[i][0] * ((i % 2 == 0) ? 1L : -1L)
						* (new MatrixArrayRC(tmpArray)).determinate();
			}
		}
		return returns;
	}
